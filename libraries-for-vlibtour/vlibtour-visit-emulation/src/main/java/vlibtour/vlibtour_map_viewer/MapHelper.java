...
public final class MapHelper {
	 ...
	 * @param name  the name of the tourist that is used to search for the position
	 *              and is also used for the text of the marker dot on the map.
	 * @return the new marker dot.
	 */
	public static MapMarkerDot addTouristOnMap(final BasicMap map, final Color color, final Font font,
			final String name) {
		return addMarkerDotOnMap(map,
				GraphOfPositionsForEmulation.getCurrentPosition(name).getGpsLocation().getLatitude(),
				GraphOfPositionsForEmulation.getCurrentPosition(name).getGpsLocation().getLongitude(), color, font,
				name);
	}
	 ...
	 */
	public static void moveTouristOnMap(final MapMarkerDot dot) {
