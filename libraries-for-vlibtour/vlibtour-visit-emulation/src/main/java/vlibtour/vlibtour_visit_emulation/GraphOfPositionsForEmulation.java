 ...
 * e.g. {@link #setStartingPosition(String, Position)},
 * {@link #stepInCurrentPath(String)}, or
 * {@link #isAtLastPositionOfCurrentPath(String)}.
 * <p>
 * Please consult the class
 * {@link vlibtour.vlibtour_visit_emulation.VisitManagementMethodsHelper} that
 * uses the data structures and the methods of this class.
 * 
 * @author Denis Conan
 */
public final class GraphOfPositionsForEmulation {
	...
	/**
	 * the integer that allows the different users to choose different paths to the
	 * next POI when there are several possible paths. This is to generate some
	 * indeterminism in the scenarios.
	 */
	private static int choiceOfPath;
	/**
	 ...
	 */
	public static synchronized void setAdjacencySets(final Map<Position, Set<Position>> adjSets) {
		adjacencySets = adjSets;
	}

	/**
	 * gets the position that matches with a given name.
	 * 
	 * @param name the name of the position to search for.
	 * @return the position found.
	 */
	public static synchronized Position getPositionOfGivenName(final String name) {
		return adjacencySets.entrySet().stream().filter(e -> e.getKey().equals(new Position(name, null))).findFirst()
				.orElseThrow(IllegalStateException::new).getKey();
	}

	/**
	 * gets the current path of a user, that the sequence of positions to the next
	 * POI.
	 * 
	 * @param user the identifier of the user.
	 * @return the current path of the user.
	 */
	public static synchronized List<Position> getCurrentPath(final String user) {
		if (currentPaths.get(user) == null) {
			throw new IllegalArgumentException("user " + user + " has no current path set (visit not started)");
		}
		return currentPaths.get(user);
	}
	 ...
	 */
	public static synchronized Position getNextPOIPosition(final String user) {
		if (currentPaths.get(user) == null) {
			throw new IllegalArgumentException("user " + user + " has no current path set (visit not started)");
		}
	...
	/**
	 * gets the current position of a user. This method shall not be called before
	 * the call to the method {@link #setStartingPosition(String, Position)}.
	 * 
	 * @param user the identifier of the user.
	 * @return the current position of the user.
	 */
	public static synchronized Position getCurrentPosition(final String user) {
		return currentPositionOfUsers.get(user);
	}

	/**
	 * sets the current position of the given user.
	 * 
	 * @param user     the identifier of the user.
	 * @param position the new position.
	 */
	public static synchronized void setCurrentPosition(final String user, final Position position) {
		currentPositionOfUsers.put(user, position);
	}

	/**
	 * states whether the given user has arrived at the last position of the current
	 * path, that is at the POI of the current path.
	 * 
	 * @param user the identifier of the user.
	 * @return the boolean indicating whether the visit is terminated.
	 */
	public static synchronized boolean isAtLastPositionOfCurrentPath(final String user) {
		...
		return currentPaths.get(user).get(currentPaths.get(user).size() - 1).equals(getCurrentPosition(user));
	}

	/**
	 * initialises the attribute {@code choiceOfPath} for the indeterminism in
	 * choosing the path.
	 */
	public static void setChoiceOfPath(final int initValue) {
		choiceOfPath = initValue;
	}
	 ...
	 * @param destination the destination position.
	 */
	public static synchronized void setAPathTo(final String user, final Position destination) {
		if (destination == null) {
		...
		if (LOG_ON && EMULATION.isDebugEnabled()) {
			EMULATION.debug(user + ": possible paths to the next POI = " + possiblePaths);
		}
		int choice = choiceOfPath++ % possiblePaths.size(); // the source of indeterminism
		currentPaths.put(user, possiblePaths.get(choice));
		if (LOG_ON && EMULATION.isInfoEnabled()) {
			EMULATION.info(user + ": path to next POI = "
					+ currentPaths.get(user).stream().map(Position::getName).collect(Collectors.joining(",")));
		}
		System.out.println(user + ": path to next POI = "
				+ currentPaths.get(user).stream().map(Position::getName).collect(Collectors.joining(",")));
	}
	 ...
	 */
	public static synchronized void setStartingPosition(final String user, final Position position) {
		if (currentPositionOfUsers.get(user) != null) {
	...
	/**
	 * steps to the next position in the current path.
	 * 
	 * @param user the identifier of the user.
	 * @return the new position of the user, or the same if the end of the path is
	 *         already reached.
	 */
	public static synchronized Position stepInCurrentPath(final String user) {
		if (adjacencySets == null || adjacencySets.isEmpty()) {
			throw new IllegalStateException("There is no graph of positions");
		}
		if (currentPaths.get(user) == null) {
		...
		if (isAtLastPositionOfCurrentPath(user)) {
			if (LOG_ON && EMULATION.isEnabledFor(Level.DEBUG)) {
				EMULATION.warn(user + ": already at the end of the current path (return the same position)");
			}
		} else {
			int indexNext = getCurrentPath(user).indexOf(currentPositionOfUsers.get(user)) + 1;
			setCurrentPosition(user, getCurrentPath(user).get(indexNext));
			if (LOG_ON && EMULATION.isInfoEnabled()) {
				EMULATION.info(user + ": step in current path, new position = " + getCurrentPath(user).get(indexNext));
			}
		}
